...
case $1 in
  ...
  10) cat complex.json | jq '.[].government.state | sort_by(.name)' ;;
  11) cat complex.json | jq '.[].government.state | group_by(.name)' ;;
  12) cat complex.json | jq '.[].government.state | length' ;;
  13) cat complex.json | jq '.[].government.state | map(.name) | unique' ;;
  ...
  20) cat complex.json | jq '.[].school.students[] | if .age > 30 then "senior" else "junior" end' ;;
