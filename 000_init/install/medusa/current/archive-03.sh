...
if ! PGPASSWORD="$DB_PASS" psql -h "$DB_HOST" -U "$DB_USER" -p "$DB_PORT" -d postgres -c '\\l' >/dev/null 2>&1; then
  ...
  if sudo -n true 2>/dev/null; then
    # Use a here-document to safely pass multi-line SQL without complex escaping
    log "   Creating user '$DB_USER' and granting privileges..."
    sudo -u postgres psql -v ON_ERROR_STOP=1 <<-EOSQL 2>>"$LOG_FILE"
      DO \$\$
      BEGIN
...
EOSQL
    if [ $? -ne 0 ]; then
        log "   ⚠️ Warning: creating DB user failed"
    else
        log "   ✅ PostgreSQL user '$DB_USER' created/verified"
    fi
  else
...
if [ "${CLEAN_DATABASE:-true}" = "true" ]; then
  log "   EXECUTING: DROP DATABASE IF EXISTS $DB_NAME;"
  sudo -u postgres psql -c "DROP DATABASE IF EXISTS $DB_NAME;" 2>>"$LOG_FILE"
  log "   EXECUTING: CREATE DATABASE $DB_NAME OWNER $DB_USER;"
...
if [ ! -f package.json ]; then
  # TARGETED CLEANUP: Remove files that would block the clone (like the .env file created in step 6)
  if [ -f .env ]; then
    log "   ⚠️ Removing existing .env to allow Git clone into non-empty directory."
    sudo -u "$RUN_USER" rm -f .env 2>>"$LOG_FILE"
  fi
